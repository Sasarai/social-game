...
@Component({
    selector: 'jhi-sphere-sg',
    templateUrl: './sphere-sg.component.html'
})
...
currentAccount: Account;
    ...
    transition() {
        this.router.navigate(['/sphere-sg'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/sphere-sg', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: SphereSg) {
        return item.id;
    }
    registerChangeInSpheres() {
        this.eventSubscriber = this.eventManager.subscribe('sphereListModification', (response) => this.loadAll());
    }
